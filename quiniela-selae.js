...
const app = express();
const PORT = process.env.PORT || 3001; // puerto distinto para no chocar

app.get('/api/quiniela-selae.js', async (req, res) => {
  ...
  try {
    ...
    // Transformar partidos a la forma que espera el frontend
    const matches = partidosRaw.map(p => ({
      ...
      status: 'FINISHED' // SELAE solo devuelve resultados
    }));
...
app.listen(PORT, () => console.log(`Servidor Quiniela SELAE activo en puerto ${PORT}`));
